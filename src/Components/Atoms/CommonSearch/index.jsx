...
const CommonSearch = ({
  ...
  const handleFilterClick = (data) => {
    ...
    if (index !== -1) {
      let filterData = selectedFilter.filter(
        (filter_id) => filter_id !== data?._id
      );
    ...
    <>
      ...
      >
        <CustomModalBody>
          {categoryData?.length ? (
            <span
              className="d-flex justify-content-end align-items-center reset-class"
              onClick={() => setSelectedFilter([])}
            >
              <FontAwesomeIcon icon={faRefresh} /> &nbsp;{" "}
              {CONSTANTS_STRINGS.RESET}
            </span>
          ) : null}
              ...
              {recipeCategoryRed ? (
                recipeCategoryRed.map((item, ind) => {
                  return (
                    <div
                      key={item?._id}
                      className={`my-chip-class my-3 `}
                      onClick={() => handleFilterClick(item)}
                    >
                      <span
                        className={`chip-data ${
                          selectedFilter.indexOf(item?._id) !== -1
                            ? "active"
                            : ""
                        } `}
                      >
          ...
          >
            {totalRecipeCount || 0} results found{" "}
          </div>
        </CustomModalBody>
      </ReactModal>
      ...
      <div className="row">
        <div className="col-lg-6 mx-auto">
          <div className="search-class-container">
            ...
            />
            <span
              className="filter-class image-em"
